...
class Period():
    ...
    def __init__(self, constructs: Constructs, data: pd.DataFrame):
        if len(data) == 0:
        ...
        missing = self.constructs.missing(self.constructs.existing())

        for key, maker in Period.construct_makers.items():
            if key in missing:
                self.data = pd.concat([
                    self.data, 
        ...
        assert len(missing(self.existing_constructs())) == 0
